...
<footer class="footer <?php echo esc_attr( ! $data['show_widgets'] ? 'footer-mini' : '' ) ?>">
	<div class="container">
		<?php if ( $data['show_widgets'] && is_active_sidebar('footer') ): ?>
			<?php wp_print_styles('mylisting-wp-widgets') ?>
			<div class="row">
				<?php
						...
						<?php if ($data['show_footer_menu']): ?>
							<div class="col-md-12 col-sm-12 col-xs-12 social-links">
								<?php wp_nav_menu([
									...
									'items_wrap' => '<ul id="%1$s" class="%2$s social-nav no-list-style">%3$s</ul>'
									]); ?>
							</div>
						<?php endif ?>
