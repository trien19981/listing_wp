...
class Listing_Type_Config {
	...
	public function get_settings() {
		...
		$config['claim_form']['used'] = array_values($this->validate_claim_form_fields( $config['claim_form']['used'] ));
	...
	public function validate_claim_form_fields( $fields ) {
		$fields_array = [];
		foreach ( (array) $fields as $key => $fieldarr ) {
			$fieldclass = sprintf( '\MyListing\Src\Claims\Claim_Fields\%s_Field', c27()->file2class( $fieldarr['type'] ) );
			if ( ! class_exists( $fieldclass ) ) {
				continue;
			}
			...
			$fields_array[ $fieldarr['slug'] ] = $field;
		}
