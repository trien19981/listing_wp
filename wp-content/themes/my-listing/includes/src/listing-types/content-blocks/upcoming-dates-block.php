...
class Upcoming_Dates_Block extends Base_Block {
	...
	public function get_dates() {
		$dates = [];
		$now_stamp = strtotime(current_datetime()->format('Y-m-d H:i:s'));
		...
		if ( $field->get_type() === 'date' ) {
			...
			if ( ! empty( $date ) && strtotime( $date ) ) {
				$dates[] = [
					...
					'is_over' => $now_stamp > strtotime( $date, $now_stamp ),
					'type' => 'date',
				];
			}
		}
		...
		if ( $field->get_type() === 'recurring-date' ) {
			...
			foreach ( $dates as $key => $date ) {
				...
				$dates[$key]['is_over'] = $now_stamp > strtotime( $date['end'], $now_stamp );
				$dates[$key]['type'] = 'recurring-date';
			}
		}
