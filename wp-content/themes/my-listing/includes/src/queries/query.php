...
class Query {
			...
			foreach ( $query->posts as $post ) {
				...
				if ( get_post_type($post_id) === 'job_listing' ) {
					...
					c27()->get_partial( 'post-preview', [
						'wrap_in' => function_exists( 'is_buddypress' ) && !is_buddypress() ? 'col-md-4 col-sm-6 col-xs-12 ' : 'col-md-4 col-sm-6 col-xs-12 grid-item',
						'post_id' => $post_id,
					] );
				}
			}
	...
	public function keyword_search( $search ) {
		...
		// Searching in the new locations table
		if ( ! empty( $searchable_meta_keys ) && in_array( '_job_location', $searchable_meta_keys ) ) {
			$conditions[] = "{$wpdb->posts}.ID IN (	SELECT listing_id FROM {$wpdb->prefix}mylisting_locations WHERE address LIKE '%" . esc_sql( $keyword ) . "%' )";
		}
