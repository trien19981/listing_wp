...
class Similar_Listings extends Query {
    ...
    public function run( $listing_id ) {
        ...
        if ( isset( $settings[$key] ) && (bool) $settings[$key] === true && ( $tax = (array) $listing->get_field( $key ) ) ) {
            $query_args['tax_query'][] = [
                'taxonomy' => $key,
                'field' => 'term_id',
                'terms' => array_filter( array_map( function( $term ) {
        ...
        if ( $settings['orderby'] === 'proximity' ) {
            $earth_radius = 6371; // km

            if ( !$listing->get_field( 'location' ) ) {
            	return;
            } else {
            	$latitude = $listing->get_field( 'location' )[0]['lat'];
            	$longitude = $listing->get_field( 'location' )[0]['lng'];
            }
