...
class View_Listings_Screen {
	...
	public function add_filters() {
		...
		// output listing type filter
		$selected = ! empty( $_GET['filter_by_type'] ) ? new \MyListing\Src\Listing_Type( get_page_by_path( $_GET['filter_by_type'], OBJECT, 'case27_listing_type' ) ) : false;
		?>
		<select data-mylisting-ajax="true" data-mylisting-ajax-url="get_listing_types" class="custom-select" name="filter_by_type" id="filter_by_type"	placeholder="<?php echo esc_attr( _x( 'Select listing type', 'WP Admin > All Listings Table', 'my-listing' ) ) ?>">
			<option></option>
			<?php if ( $selected instanceof \MyListing\Src\Listing_Type ): ?>
			<option value="<?php echo $selected->get_slug(); ?>" selected="selected"><?php echo $selected->get_plural_name(); ?></option>
			<?php endif ?>
		</select>
		<?php
