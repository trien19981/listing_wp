	...
	public function handle() {
		...
		try {
			...
			$post_status = ! empty( $_REQUEST['post-status'] )
				...
				$post_type = ! empty( $_REQUEST['post_type'] ) ? $_REQUEST['post_type'] : 'job_listing';
				if ( ! is_array( $post_type ) ) {
					$post_type = array( sanitize_text_field( $post_type ) );
				}
				$allowed_post_types = array( 'job_listing', 'post', 'page', 'case27_listing_type' );
				$post_type = array_intersect( $post_type, $allowed_post_types );
