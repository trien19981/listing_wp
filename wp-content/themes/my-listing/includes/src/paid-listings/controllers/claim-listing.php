...
class Claim_Listing {
	...
	public function subscription_processed( $listing, $package ) {
		...
		$form_fields = $listing->type->get_claim_fields();

		if ( ! empty( $form_fields ) ) {
			$cache_key = sprintf( 'mylisting_claim_fields_%s', $listing->get_id() );
			$cache_forms = get_transient( $cache_key );

			if ( $cache_forms !== false && $claim_id ) {
				$this->update_claim_data( $cache_forms, $claim_id );
	...
	public function product_selected( $listing, $product ) {
		...
		$form_fields = \MyListing\Src\Claims\Claim_Listing_Form::instance()->get_form_fields();

		if ( ! empty( $form_fields ) ) {
			$cache_key = sprintf( 'mylisting_claim_fields_%s', $listing->get_id() );
			$expiration = 1 * HOUR_IN_SECONDS;

			set_transient( $cache_key, $form_fields, $expiration );
		}
	...
	public function use_available_package( $listing, $package ) {
		...
		if ( ! empty( $form_fields ) ) {
			\MyListing\Src\Claims\Claim_Listing_Form::instance()->update_claim_data( $claim_id );

			// valid, perform redirect
			$redirect_url = add_query_arg( '_claim_id', $claim_id, wc_get_account_endpoint_url( _x( 'claim-requests', 'Claims user dashboard page slug', 'my-listing' ) ) );
		} else {
			// success, redirect to claim info page
			$claim_url = add_query_arg( 'listing_id', $listing->get_id(), get_permalink() );
			$redirect_url = esc_url_raw( add_query_arg( '_claim_id', absint( $claim_id ), $claim_url ) );
		}
	...
	public function update_claim_data( $fields, $claim_id ) {
		foreach ( $fields as $key => $field ) {
			$field->set_claim( $claim_id );
			$_POST[$field->get_key()] = $field->props['value'];
			$field->update();
