	...
	foreach ( $files as $file ) {
		...
		if ( is_serialized( $url_or_path ) ) {
			...
			foreach ( $url_or_path as $index => $url ) {
				...
				$unique_filename = $path_info['filename'] . '-' . uniqid() . '.' . $path_info['extension'];
				...
				$attachment_id = \PMXI_API::upload_image(
					$field->listing->get_id(), $url, $download_image, $log, true, $unique_filename, $file_type, true, $import['articleData'], $import
				);
			...
			$parsed_url = parse_url($url_or_path);
			$path_info = pathinfo($parsed_url['path']);
			$unique_filename = $path_info['filename'] . '-' . uniqid() . '.' . $path_info['extension'];
			...
			$attachment_id = \PMXI_API::upload_image(
				$field->listing->get_id(), $url_or_path, $download_image, $log, true, $unique_filename, $file_type, true, $import['articleData'], $import
			);
