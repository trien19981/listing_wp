...
class Dashboard_Listings_Controller extends Base_Controller {
	...
	protected function display_edit_action( $listing ) {
		// Add a filter hook to enable/disable the edit button.
		$show_edit = apply_filters( 'mylisting/user-listings/show_edit_button', true );
	...
	protected function display_delete_action( $listing ) {
		// Add a filter hook to enable/disable the delete button.
		$show_delete = apply_filters( 'mylisting/user-listings/show_delete_button', true );

		if ( ($listing->get_status() === 'pending_payment' && ! empty( $this->pending_orders[ $listing->get_id() ] ) ) || ! $show_delete ) {
			return;
		}
	...
	protected function display_pending_payment_actions( $listing ) {
		...
		<?php } else {
			$resume_url = add_query_arg( [
				'listing_type' => isset($listing) && isset($listing->type) ? $listing->type->get_slug() : '',
				'job_id' => $listing->get_id(),
			], $add_listing_page );
	...
	protected function display_listing_status_action( $listing ) {
		// Add a filter hook to enable/disable listing status button.
		$show_status = apply_filters( 'mylisting/user-listings/show_status_button', true );

		if ( ! in_array( $listing->get_status(), ['publish', 'unpublish'], true ) || ! $show_status ) {
			return;
		}
