...
class Button {
	...
	public function add_shortcode( $atts, $content = null ) {
		...
		return do_shortcode( sprintf(
				'<a href="%2$s" %8$s class="buttons button-%3$s %4$s %5$s %6$s">%1$s %7$s</a>',
	...
	public function output_options() { ?>
		...
		<div class="form-group">
			<label><?php _e( 'Open in new tab', 'my-listing' ) ?></label>
			<select v-model="shortcode.attributes.target">
				<option value="_blank"><?php _e( 'Yes', 'my-listing' ) ?></option>
				<option value=""><?php _e( 'No', 'my-listing' ) ?></option>
			</select>
		</div>
