...
class Stats {
	...
	public function display_stats_action( $listing ) {
		// Add a filter hook to enable/disable the stats button.
		$show_stats = apply_filters( 'mylisting/user-listings/show_stats_button', true );

		if ( $listing->get_status() !== 'publish' || ! $show_stats ) {
			return;
		}
