...
class Breadcrumbs {
    ...
    public function __construct( $templates = array(), $options = array(), $strings = array(), $autorun = true ) {
        $this->templates = wp_parse_args(
            ...
            array(
                'link' => '<a href="%s">%s</a>',
                'current' => '<span class="c">%s %s</span>',
                'standard' => '<span class="s">%s %s</span>',
                'before' => '<nav>',
                'after' => '</nav>'
            )
        );
    ...
    protected function template( $item, $type = 'standard' ) {
        ...
        switch ( $type ) {
            case'link':
                ...
                break;
            case 'current':
                return sprintf( $this->templates['current'], $item, '<meta itemprop="position" content="'. $this->position++ .'" />' );
                break;
            case 'standard':
                return sprintf( $this->templates['standard'], $item, '<meta itemprop="position" content="'. $this->position++ .'" />' );
                break;
        }
    }
