...
function display_recaptcha() {
	...
	wp_enqueue_script( 'mylisting-recaptcha' );

	if ( mylisting_get_setting( 'mylisting_recaptcha_type' ) === 'v2' ) {
...
function validate_recaptcha() {
	...
	$captcha_type = mylisting_get_setting( 'mylisting_recaptcha_type' );
	$response = null;

	if ( empty( $secret_key ) ) {
		throw new \Exception( __( 'Missing security token.', 'my-listing' ) );
	}
	...
	if ( ! $json || ! $json->success ) {
		throw new \Exception( __( 'Could not verify captcha.', 'my-listing' ) );
	}

	if ( $captcha_type === 'v3' ) {
		$score_threshold = mylisting_get_setting('recaptcha_score_threshold') ?: 0.5;
		if ( $json->score < $score_threshold) {
			throw new \Exception( __( 'Could not verify captcha due to low score.', 'my-listing' ) );
		}
	}
}
...
function is_edit_mode() {
	return \MyListing\is_elementor_active() && \Elementor\Plugin::$instance->editor->is_edit_mode();
}

function is_preview_mode() {
	return \MyListing\is_elementor_active() && \Elementor\Plugin::$instance->preview->is_preview_mode();
}
...
function get_svg( $filename ) {
	return \MyListing\svg( $filename, false );
}
...
function allow_author_reviews( $listing_id ) {
	$listing = \MyListing\Src\Listing::get( $listing_id );
	if ( ! ( $listing && $listing->type ) ) {
		return false;
	}

	return $listing->type->allow_author_reviews();
}
...
function duplicate_listing( $listing_id ) {
	...
	foreach ( $fields as $field ) {
		...
		if ( $field->get_type() === 'related-listing' ) {
			...
			$_POST[ $field->get_key() ] = ! empty( $old_field ) ? $old_field->get_value() : '';
			$field->update();
		}
		...
		elseif ( $field->get_type() === 'term-select' ) {
			...
			$terms = ! empty( $old_field ) ? $old_field->get_value() : [];
		...
		elseif ( $field->get_type() === 'file' ) {
			...
			$upload_files = [];
			if ( ! empty( $old_field ) ) {
				foreach ( (array) $old_field->get_value() as $file_guid ) {
					if ( is_numeric( $file_guid ) ) {
						continue;
					}

					$tmp = download_url( $file_guid );
					if ( is_wp_error( $tmp ) ) {
						continue;
					}

					$attachment_id = c27()->get_attachment_by_guid( $file_guid );
					if ( ! $attachment_id ) {
						continue;
					}

					$file_array             = [];
					$file_array['name']     = basename( $file_guid );
					$file_array['tmp_name'] = $tmp;
				// "Upload" to the media collection
					$new_attachment_id = media_handle_sideload( $file_array, $new_listing->get_id() );
					...
					$upload_files[] = 'b64:'.base64_encode( $url );
				}
			}
			
			$_POST[ 'current_'.$field->get_key() ] = $upload_files;
			$field->update();
		}
