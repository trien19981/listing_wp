    ...
    RepeaterUploadFiles = function( selfRepeaterItem ) {
                ...
                selfRepeaterItem.find('.wp-job-manager-file-upload').each(function(){
                    ...
                    var max_count = parseInt( $(this).data('max_count'), 1 );
                            ...
                            if ( allowed_size ) {
                            	if ( ! isNaN(allowed_size) && allowed_size < data.files[0].size/1024 ) {
                            		window.alert( CASE27.l10n.file_limit_exceeded.replace( '%s', data.files[0].name ).replace( '%d', allowed_size + 'KB' ) );
                            		return;
                            	}
                            }
