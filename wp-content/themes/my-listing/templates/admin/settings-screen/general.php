...
<div id="captcha-config">
	...
	<div class="form-group mb30">
		<h4 class="m-heading mb5">Recaptcha type</h4>
		<p class="mt0 mb10">Choose between reCAPTCHA v2 and v3</p>
		<?php
			$recaptcha_type = $this->get_setting( 'mylisting_recaptcha_type' ) ?? 'v2';
		?>
		<div class="select-wrapper dib">
			<select name="<?php echo esc_attr( 'mylisting_recaptcha_type' ) ?>">
				<option value="v2" <?php selected( 'v2', $recaptcha_type ) ?>>v2</option>
				<option value="v3" <?php selected( 'v3', $recaptcha_type ) ?>>v3</option>
			</select>
		</div>
	</div>

	<?php if ( $recaptcha_type === 'v3' ): ?>
		<div class="form-group mb30 recaptcha-score">
			<h4 class="m-heading mb5">Recaptcha score threshold</h4>
			<p class="mt0 mb10">
				<?php printf(
					_x( 'Read more about recaptcha score <a target="_blank" href="%s">here</a>.', 'WP Admin > Listings > Settings', 'my-listing' ),
					'https://developers.google.com/recaptcha/docs/v3#interpreting_the_score'
				) ?>
			</p>
			<input type="number" class="m-input" style="max-width: 170px;" step="0.1" min="0" max="1" placeholder="0.5" name="recaptcha_score_threshold" value="<?php echo esc_attr( $this->get_setting( 'recaptcha_score_threshold' ) ) ?>">
		</div>
	<?php endif ?>
