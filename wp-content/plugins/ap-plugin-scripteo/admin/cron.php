...
?>
<h2>
		<span class="dashicons dashicons-calendar-alt"></span> Schedule Tasks
<?php if ( get_option('bsa_pro_u'.$zxzcow.'pd'.'at'.$zxzcow.'e'.'_s'.$zxzcow.'ta'.'tus') != 'i'.$zxzcow.'n'.'v'.$zxzcow.'a'.'l'.$zxzcow.'i'.$zxzcow.'d' ): ?>
	<?php if ( isset($_GET['bsa-form']) ): ?>
		<p><span class="dashicons dashicon-14 dashicons-arrow-left-alt"></span> <a href="<?php echo admin_url(); ?>admin.php?page=bsa-pro-sub-menu-cron">back to <strong>tasks</strong></a></p>
	<?php else: ?>
...
<?php $asdpojsad = ''; ?>
<?php if ( get_option('bsa_pro_u'.$asdpojsad.'pd'.'at'.$asdpojsad.'e'.'_s'.$asdpojsad.'ta'.'tus') == 'i'.$asdpojsad.'n'.'v'.$asdpojsad.'a'.'l'.$asdpojsad.'i'.$asdpojsad.'d' ): ?>

	<tr class="bsaAdminTable showAdvanced">
		<td colspan="2">
			<div class="bsaLockedContent bsaLockedWhite">
				<strong>Trial Version</strong><br>
				Use purchase code to unlock Schedule Tasks and other features.<br><br>
...
<?php else: ?>
	...
	<?php else: ?>
		...
		<table class="wp-list-table widefat bsaListTable">
			...
			if ( is_array($getTasks) && count($getTasks) > 0 && $tasksPagination->getTasksPages() && $tasksPagination->getTasksPages() != 'not_found' ) {
				...
				if ( is_array($getTasks) && count($getTasks) > 40 ): ?>
					<tr>
						<td colspan="6">
							<?php
