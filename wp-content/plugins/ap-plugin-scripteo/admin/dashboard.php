...
if ( $model->validationAccept() ) {
	...
	echo '
		<div class="updated settings-error" id="setting-error-settings_updated">
			<p><div class="bsaLoader"></div><strong>Agency Withdrawal has been marked as paid.</strong></p>
		</div>';
	...
	echo '
		<div class="updated settings-error" id="setting-error-settings_updated">
			<p><div class="bsaLoader"></div><strong>Affiliate Withdrawal has been marked as paid.</strong></p>
		</div>';
	...
	<?php $rtefsd = ''; ?>
	<?php $deregister = null; ?>
	<?php if ( isset($_POST['a']) && $_POST['a'] == 'deactivation' && isset($_POST['p']) && $_POST['p'] != '' ): ?>
		<?php $domain = str_replace('/', '', str_replace('www.', '', str_replace('http://', '', str_replace('https://', '', get_site_url())))); ?>
		<?php $deregister = wp_remote_get('ht'.'tp'.'s:/'.'/u'.'pd'.'ate.s'.'c'.'r'.'i'.'p'.'t'.'e'.'o.i'.'n'.'f'.'o/?a=deactivation&p='.$_POST['p'].'&s='.$domain) ?>
		<?php update_option('bsa_pro_plugin_purchase_code', '') ?>
		<?php update_option('b'.$rtefsd.'sa'.$rtefsd.'_p'.$rtefsd.'ro_u'.'pd'.'at'.$rtefsd.'e'.'_s'.$rtefsd.'ta'.'tus', 'i'.'n'.'val'.'id') ?>
		<?php if ( isset($deregister['body']) && $deregister['body'] != '' ): ?>
			<div class="notice notice-message">
				<p><?php echo $deregister['body'] ?></p>
			</div>
		<?php endif; ?>
	<?php else: ?>
...
<?php if ( get_option('bsa_pro_plugin_version') < '4.43' ): ?>
	<div class="updated error" style="float:left;">
			...
			<tr class="<?php echo $alternate; ?>">
				...
				<td class="post-title page-title column-title">
					...
					<div class="row-actions">
						...
						<form action="" method="post">
							...
							<input type="hidden" value="<?php echo $entry['id']; ?>" name="orderId">
								<span class="bsaProActionBtn bsaBlockBtn"><input type="submit" value="Reject"
				...
				<td>
					...
					if ($entry['ad_model'] == 'cpd') {
						...
						$diffTime = date('d/m/Y (H:i)', time() + $diff); // d M Y (H:m:s)
					} else {
				...
				<td>
					...
					<?php if ( isset($entry['starts']) && $entry['starts'] > time() ): ?>
						- - - -<br>
						Start Date <strong><?php echo date('d/m/Y (H:i)', $entry['starts']); ?></strong><br>
					<?php endif; ?>
					<?php if ( isset($entry['ends']) && $entry['ends'] > time() ): ?>
						- - - -<br>
						Ends <strong><?php echo date('d/m/Y (H:i)', $entry['ends']); ?></strong><br>
					<?php endif; ?>
...
<h3>Pending Affiliate Withdrawals (<a href="1.envato.market/M6K6K" target="_blank">Affiliate Program Add-on</a>)</h3>
<table class="wp-list-table widefat bsaListTable">
	<thead>
	<tr>
	...
	<tbody>
	<?php
	if (is_array($getAffiliateWithdrawals) && count($getAffiliateWithdrawals) > 0) {
		foreach ($getAffiliateWithdrawals as $key => $entry) {
			...
			<tr class="<?php echo $alternate; ?>">
				...
				<td>
					<?php echo (($entry['request_time'] > 0) ? '<strong>'.date('d/m/Y', $entry['request_time']).'</strong> '.date('H:i', $entry['request_time']) : '-'); // Y M d ?>
				</td>
				<td>
...
<h3>Pending Sites (<a href="https://1.envato.market/M6K6K" target="_blank">Marketing Agency Add-on</a>)</h3>
<table class="wp-list-table widefat bsaListTable">
	<thead>
	<tr>
		<th></th>
	...
	<tbody>
	<?php
	if (is_array($getSites) && count($getSites) > 0) {
		foreach ($getSites as $key => $entry) {
			...
			<tr class="<?php echo $alternate; ?>">
				<td class="bsaAdminImg">
					<img class="bsaAdminThumb" src="<?php echo ( $entry['thumb'] != '' ) ? bsa_upload_url(null, $entry['thumb']) : plugin_dir_url( __DIR__ ).'frontend/img/example.png'; ?>">
				</td>
				<td class="post-title page-title column-title">
...
<h3>Pending Agency Withdrawals (<a href="https://1.envato.market/M6K6K" target="_blank">Marketing Agency Add-on</a>)</h3>
<table class="wp-list-table widefat bsaListTable">
	<thead>
	<tr>
		<th>ID</th>
	...
	<tbody>
	<?php
	if (is_array($getWithdrawals) && count($getWithdrawals) > 0) {
...
<table class="wp-list-table widefat bsaListTable">
	...
	<tbody>
	<?php
	if (is_array($getLastAds) && count($getLastAds) > 0) {
