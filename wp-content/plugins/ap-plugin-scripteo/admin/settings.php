...
function validValue($variableName)
{
	if($_SERVER["REQUEST_METHOD"] == "POST") {
		echo esc_html__( $_POST[$variableName], 'bsa-pro' );
	} else {
...
<?php bsaProCheckUpdate('empty'); ?>

<?php $rtefsd = ''; ?>
<?php if ( get_option('b'.$rtefsd.'sa'.$rtefsd.'_p'.$rtefsd.'ro_u'.'pd'.'at'.$rtefsd.'e'.'_s'.$rtefsd.'ta'.'tus') != 'i'.$rtefsd.'n'.'v'.$rtefsd.'a'.'l'.$rtefsd.'i'.$rtefsd.'d' ): ?>
	<div class="error error-warning bsaDeactivateLicenseNow" style="display: none;">
		<p>Deacivate the Ads Pro license from this site to use Purchase Code on the other one.</p>
		<form action="<?php echo admin_url(); ?>admin.php?page=bsa-pro-sub-menu" method="post">
			<input type="hidden" name="a" value="deactivation" />
			<input type="hidden" name="p" value="<?php echo get_option('bsa_pro_plugin_purchase_code') ?>" />
			<input type="submit" class="button button-primary" value="Deactivate Now" />
		</form><br>
	</div>
<?php endif; ?>

	<h2 class="nav-tab-white nav-tab-wrapper">
		<a href="#bsaPayment" class="nav-tab nav-tab-active" data-group="bsaTabPayment">General</a>
		...
		<a href="#bsaAffiliate" class="nav-tab" data-group="bsaTabAffiliateProgram">Affiliate Program</a>
		<a href="#bsaAddOn" class="nav-tab" data-group="bsaTabMarketingAgency">Marketing Agency</a>
	</h2>
		...
		<table class="bsaAdminTable bsaMarTopNull form-table">
			<tbody id="bsaPayment" class="bsaTabPayment bsaTbody">
				<tr class="bsaBottomLine">
					...
					<td>
						<input type="text" class="regular-text code" value="<?php validValue('purchase_code'); ?>" id="purchase_code" name="purchase_code">
						<?php if ( strlen(get_option('bsa_pro_plugin_purchase_code')) == 36 ): ?>
							<div class="updated-message">
								<p>
									Purchase code has been saved.
									<?php if ( get_option('b'.$rtefsd.'sa'.$rtefsd.'_p'.$rtefsd.'ro_u'.'pd'.'at'.$rtefsd.'e'.'_s'.$rtefsd.'ta'.'tus') != 'i'.$rtefsd.'n'.'v'.$rtefsd.'a'.'l'.$rtefsd.'i'.$rtefsd.'d' ): ?>
										Here you can <strong class="bsaDeactivateLicense error-message">Deactivate License</strong>
									<?php endif; ?>
								</p>
							</div>
						<?php else: ?>
							<p class="description">
								<strong class="error-message">Purchase code invalid. Required to unlock all features!</strong> You can download it from <a href="https://1.envato.market/6rkOr" target="_blank">CodeCanyon</a><br>
								Where is your purchase code? <a href="https://help.market.envato.com/hc/en-us/articles/202822600-Where-Is-My-Purchase-Code-" target="_blank">Learn more</a>
							</p>
						<?php endif; ?>
					</td>
				</tr>
				...
				<tr>
					<th scope="row"><label for="paypal">PayPal Email</label>
				...
				<tr>
					...
					<td><input type="url" class="regular-text code" maxlength="1000" value="<?php validValue('ordering_form_url'); ?>" id="ordering_form_url" name="ordering_form_url">
						...
						<p class="description"><strong>Example</strong> https://your_page.com/order_ads</p></td>
				</tr>
				...
				<tr>
					...
					<td>
						<fieldset>
							<label title="symbol before"><input type="radio" <?php if(get_option('bsa_pro_plugin_'.'symbol_position') == 'before') { echo 'checked="checked"'; } ?> value="before" name="symbol_position"><strong>before</strong> price <span>(eg. <strong><?php echo get_option('bsa_pro_plugin_'.'currency_symbol') ?><?php echo bsa_number_format(1234.00) ?></strong>)</span></label><br>
							<label title="symbol after"><input type="radio" <?php if(get_option('bsa_pro_plugin_'.'symbol_position') == 'after') { echo 'checked="checked"'; } ?> value="after" name="symbol_position"><strong>after</strong> price <span>(eg. <strong><?php echo bsa_number_format(1234.00) ?><?php echo get_option('bsa_pro_plugin_'.'currency_symbol') ?></strong>)</span></label>
						</fieldset>
					</td>
				</tr>
				<tr>
					<th scope="row"><label for="price_format">Price Format</label></th>
					<td>
						<fieldset>
							<label title="format 1234.00"><input type="radio" <?php if(get_option('bsa_pro_plugin_'.'currency_format') == '2|.|') { echo 'checked="checked"'; } ?> value="2|.|" name="price_format"><strong>1234.00</strong> (default)</label><br>
							<label title="format 1234,00"><input type="radio" <?php if(get_option('bsa_pro_plugin_'.'currency_format') == '2|,|') { echo 'checked="checked"'; } ?> value="2|,|" name="price_format"><strong>1234,00</strong></label><br>
							<label title="format 1,234.00"><input type="radio" <?php if(get_option('bsa_pro_plugin_'.'currency_format') == '2|.|,') { echo 'checked="checked"'; } ?> value="2|.|," name="price_format"><strong>1,234.00</strong></label><br>
							<label title="format 1.234,00"><input type="radio" <?php if(get_option('bsa_pro_plugin_'.'currency_format') == '2|,|.') { echo 'checked="checked"'; } ?> value="2|,|." name="price_format"><strong>1.234,00</strong></label><br>
							<label title="format 1,234"><input type="radio" <?php if(get_option('bsa_pro_plugin_'.'currency_format') == '0||,') { echo 'checked="checked"'; } ?> value="0||," name="price_format"><strong>1,234</strong></label><br>
							<label title="format 1.234"><input type="radio" <?php if(get_option('bsa_pro_plugin_'.'currency_format') == '0||.') { echo 'checked="checked"'; } ?> value="0||." name="price_format"><strong>1.234</strong></label><br>
							<label title="format 1234"><input type="radio" <?php if(get_option('bsa_pro_plugin_'.'currency_format') == '0||') { echo 'checked="checked"'; } ?> value="0||" name="price_format"><strong>1234</strong></label><br>
						</fieldset>
					</td>
				</tr>
				<tr>
				...
				<tr>
					<th scope="row">Show calendar in the Order Form</th>
					<td>
						<fieldset>
                ...
                <tr>
                    <th class="bsaLast" scope="row">Ad Spaces order by</th>
                    <td class="bsaLast">
                        <fieldset>
                            <label title="id"><input type="radio" <?php validNewSelectedOpt('_advanced', 'order_by', 'id', 'checkbox'); ?> value="id" name="order_by"><strong>space id - ascending <span class="dashicons dashicons-arrow-up"></span></strong></label><br>
                            <label title="id_desc"><input type="radio" <?php validNewSelectedOpt('_advanced', 'order_by', 'id_desc', 'checkbox'); ?>value="id_desc" name="order_by"><strong>space id - descending <span class="dashicons dashicons-arrow-down"></span></strong></label><br>
                            <label title="name"><input type="radio" <?php validNewSelectedOpt('_advanced', 'order_by', 'name', 'checkbox'); ?> value="name" name="order_by"><strong>space name - ascending <span class="dashicons dashicons-arrow-up"></span></strong></label><br>
                            <label title="name_desc"><input type="radio" <?php validNewSelectedOpt('_advanced', 'order_by', 'name_desc', 'checkbox'); ?>value="name_desc" name="order_by"><strong>space name - descending <span class="dashicons dashicons-arrow-down"></span></strong></label><br>
                            <label title="title"><input type="radio" <?php validNewSelectedOpt('_advanced', 'order_by', 'title', 'checkbox'); ?> value="title" name="order_by"><strong>space title - ascending <span class="dashicons dashicons-arrow-up"></span></strong></label><br>
                            <label title="title_desc"><input type="radio" <?php validNewSelectedOpt('_advanced', 'order_by', 'title_desc', 'checkbox'); ?>value="title_desc" name="order_by"><strong>space title - descending <span class="dashicons dashicons-arrow-down"></span></strong></label>
                        </fieldset>
                    </td>
                </tr>
			</tbody>
			...
			<tr>
				<th class="bsaLast" scope="row">Delete the data when uninstalling?</th>
				<td class="bsaLast">
					<fieldset>
						<label title="no"><input type="radio" <?php if(get_option('bsa_pro_plugin_'.'installation') == 'no') { echo 'checked="checked"'; } ?> value="no" name="installation"><strong>no</strong>, keep all settings, spaces and ads</label><br>
						<label title="yes"><input type="radio" <?php if(get_option('bsa_pro_plugin_'.'installation') == 'yes') { echo 'checked="checked"'; } ?>value="yes" name="installation"><strong>yes</strong>, delete all data (settings, spaces and ads)</label><br>
						<label title="ads"><input type="radio" <?php if(get_option('bsa_pro_plugin_'.'installation') == 'ads') { echo 'checked="checked"'; } ?>value="ads" name="installation"><strong>yes</strong>, delete all ads only</label><br>
					</fieldset>
				</td>
			</tr>
			</tbody>
			...
			<tr>
				<th scope="row"><label for="username">Impact Ref Link<br>Go to <a href="https://impact.com/" target="_blank">Impact.com</a></label></th>
				<td><input type="text" class="regular-text ltr" value="<?php validValue('username'); ?>" id="username" name="username">
					<p class="description">
						<strong>Paste referral link to earn more by selling Ads Pro licenses</strong>, example: https://1.envato.market/6rkOr<br>
						You can earn 30% from each new Customer. <a href="https://www.envato.com/affiliates/how-do-i-create-a-referral-link-in-impact/" target="_blank" rel="nofollow">Learn more</a> - How to create referral link in Impact?
					</p>
				</td>
			</tr>
			<tr>
				...
				<td>
					<fieldset>
						...
						<label title="frontend"><input type="radio" <?php if(get_option('bsa_pro_plugin_'.'editable') == 'frontend') { echo 'checked="checked"'; } ?> value="frontend" name="editable"><strong>frontend</strong> (default)</label><br>
						<label title="backend-session"><input type="radio" <?php if(get_option('bsa_pro_plugin_'.'editable') == 'backend-session') { echo 'checked="checked"'; } ?> value="backend-session" name="editable"><strong>backend</strong> (php session disabled)</label><br>
						<label title="frontend-session"><input type="radio" <?php if(get_option('bsa_pro_plugin_'.'editable') == 'frontend-session') { echo 'checked="checked"'; } ?> value="frontend-session" name="editable"><strong>frontend</strong> (php session disabled)</label>
					</fieldset>
				</td>
			</tr>
			<tr>
			...
			<tr>
				<th scope="row"><label for="free_ads_limit">Price limit of Free Ads</label></th>
				<td><input type="number" class="regular-text ltr" value="<?php validValue('free_ads_limit'); ?>" id="free_ads_limit" name="free_ads_limit">
					<p class="description">
						Price limit of Ads which are available to get for Free.
					</p>
				</td>
			</tr>
			<tr style="display: none">
				<th scope="row"><label for="ga_id">Google Analytics ID</label></th>
				<td>
					<input type="text" class="regular-text ltr" value="<?php echo bsa_get_opt('admin_settings', 'ga_id'); ?>" id="ga_id" name="ga_id">
				</td>
			</tr>
			<tr>
			...
			<tr>
				...
				<td>
					...
					<p class="description"><strong>Use this option carefully because this option affect on upload folder.</strong><br>(default: ap-plugin-upload)</p>
				</td>
			</tr>
			<tr>
...
<!--				</tr>-->
			...
			<tbody id="bsaCustomization" class="bsaTabOrderForm bsaTbody" style="display:none">
				...
				<tr>
					...
					<td class="bsaLast">
						<textarea id="advanced_calendar" name="advanced_calendar" class="regular-text ltr" rows="17" cols="70"><?php echo get_option('bsa_pro_plugin_'.'advanced_calendar') ?></textarea>
					</td>
				</tr>
				<?php endif; ?>
			</tbody>
			<tbody id="bsaAffiliate" class="bsaTabAffiliateProgram bsaTbody" style="display:none">
				<tr>
					<th colspan="2">
						<h3><span class="dashicons dashicons-cart"></span> Affiliate Program Settings (<a href="https://1.envato.market/M6K6K" target="_blank">Affiliate Program Add-on</a>)</h3>
					</th>
				</tr>
				<tr>
			...
			<tr>
				<th colspan="2">
					<h3><span class="dashicons dashicons-cart"></span> Marketing Agency Settings (<a href="https://1.envato.market/M6K6K" target="_blank">Marketing Agency Add-on</a>)</h3>
				</th>
			</tr>
			<tr>
	...
	(function($){
		...
		$(function() {
			...
			$(document).ready(function(){
				...
				// deactivate license
				let bsaDeactivateLicense = $('.bsaDeactivateLicense');
				let bsaDeactivateLicenseNow = $('.bsaDeactivateLicenseNow');
				bsaDeactivateLicense.on("click", function(){
					bsaDeactivateLicenseNow.fadeIn();
				});

				let bsaSwitch = $(".bsaSwitch");
				$.switcher(".bsaSwitch");
