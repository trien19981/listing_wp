<?php
require_once dirname(__FILE__) . '/BSA_PRO_Agency_Ordering_form.php'; // require ordering form if stats
...
if ( file_exists( $report ) ) {
	echo 	'<div style="text-align: right">
			...
			<a style="margin-left: 10px;" href="' . plugin_dir_url(__FILE__) . 'pdf.php?pdf=' . substr(md5($ad_id . '1'), 1, 11) . '&ad_id=' . $ad_id . '&stats=90&ma=1" target="_blank">'.bsa_get_trans('statistics', 'last_90').'</a>
			<a style="margin-left: 10px;" href="' . plugin_dir_url(__FILE__) . 'pdf.php?pdf=' . substr(md5($ad_id . '1'), 1, 11) . '&ad_id=' . $ad_id . '&stats=30&ma=1" target="_blank">'.bsa_get_trans('statistics', 'last_30').'</a>
			<a style="margin-left: 10px;" href="' . plugin_dir_url(__FILE__) . 'pdf.php?pdf=' . substr(md5($ad_id . '1'), 1, 11) . '&ad_id=' . $ad_id . '&stats=7&ma=1" target="_blank">'.bsa_get_trans('statistics', 'last_7').'</a>
		</div>';
}
echo '	<div class="bsaChart ct-chart"></div>';
echo '<h3 class="bsaHeaderClicks">'.get_option("bsa_pro_plugin_trans_stats_clicks").'</h3>';
