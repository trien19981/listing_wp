...
<div class="bsaProOrderingForm">
	...
	if ( isset($_GET['site_id']) && $_GET['site_id'] != '' && $model->countSpaces('agency_form', $_GET['site_id']) > 0 || $eid && $hasAccess ) {
				...
				<div class="bsaProInput'.($eid ? 'Hidden' : '').'" style="'.($eid ? 'display:none' : '').'">
					...
					foreach ($spaces as $space) {
						if (in_array($space['id'], $space_verify)) {
							if ($model->countAds($space["id"]) < bsa_space($space["id"], 'max_items') || bsa_space($space["id"], 'max_items') == 1 && get_option('bsa_pro_plugin_calendar') == 'yes') {
								echo '<option value="' . $space["id"] . '" max="'.bsa_space($space["id"], 'max_items').'" s' . ((isset($_POST["space_id"]) && $_POST["space_id"] == $space["id"] or isset($_GET['sid']) && $_GET['sid'] == $space["id"]) ? 'selected="selected"' : "") . '>' . $space["name"] . '</option>';
							} else {
					...
					if ( !$eid ) {
						...
						echo '<div class="bsaProInput">
								...
								<input id="bsa_pro_buyer_email" name="bsa_buyer_email" type="email" value="'.$get_email.'" placeholder="'.get_option($bsaTrans."form_left_eg_email").'">
							</div>';
					}
					echo '
		...
		echo '
		<div class="apPluginContainer bsa-pro-col-3" style="display: block !important">
...
<?php if ( isset($_GET['site_id']) && $_GET['site_id'] != '' ): ?>
<script>
	(function($) {
		"use strict";
