...
// -- START -- TEMPLATE HTML
	...
	if ( isset($sid) && $sid > 0 ) {
		echo '<div class="bsaProItems '.bsa_space($sid, "grid_system").' '.((str_contains(bsa_space($sid, 'display_type'), 'carousel') !== false) ? 'bsa-owl-carousel bsa-owl-carousel-'.$sid : '').'" style="background-color:'.bsa_space($sid, 'ads_bg').'">'; // -- START -- ITEMS
	} else {
	...
	foreach ( $ads as $key => $ad ) {
		...
		echo '<div class="bsaProItem bsaHidden '.($col_per_row  > 0 && ($key % $col_per_row == 0) ? "bsaReset" : "").' card" data-item-id="'.(isset($ad['id']) && $ad['id'] > 0 ? $ad['id'] : 0 ).'" data-animation="'.bsa_space($sid, "animation").'" style="'.((bsa_space($sid, "animation") == "none" || bsa_space($sid, "animation") == NULL || $crop == 'ajax') ? "opacity:1;visibility:visible;" : "").'">'; // -- START -- ITEM
			...
			if ( $ad['url'] != '' ) {
				...
				if ( isset($example) ) { // url to form if example in ad space
					...
					if ( isset($type) && $type == 'agency' ) {
						...
						echo '<a href="'.$form_url.( (strpos($form_url, '?')) ? '&' : '?' ).'bsa_pro_id='.$ad['id'].'&bsa_pro_url=1" target="_blank" class="bsaProItem__url bsaProItemInner__btn btn-floating btn-large waves-effect waves-light" style="background-color:'.bsa_space($sid, 'ad_extra_color_1').'">';
					}
				}
