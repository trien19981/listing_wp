...
// -- START -- TEMPLATE HTML
	...
	foreach ( $ads as $key => $ad ) {
		...
		echo '<div class="bsaProItem bsaHidden '.($col_per_row  > 0 && ($key % $col_per_row == 0) ? "bsaReset" : "").'" data-item-id="'.(isset($ad['id']) && $ad['id'] > 0 ? $ad['id'] : 0 ).'" data-item-id="'.(isset($ad['id']) && $ad['id'] > 0 ? $ad['id'] : 0 ).'" data-animation="'.bsa_space($sid, "animation").'" style="'.((bsa_space($sid, "animation") == "none" || bsa_space($sid, "animation") == NULL || $crop == 'ajax') ? "opacity:1;visibility:visible;" : "").'">'; // -- START -- ITEM
		...
		echo "<div class=\"bsaProItemInner__img\" style=\"background-image: url('".bsa_crop_tool($crop, bsa_upload_url(null, $ad['img']), 125, 125)."')\"></div>"; // -- ITEM -- IMG
