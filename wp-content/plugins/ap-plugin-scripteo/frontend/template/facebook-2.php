...
// -- START -- TEMPLATE HTML
	...
	foreach ( $ads as $key => $ad ) {
		...
		echo "<div class=\"bsaProItemInner__img\" style=\"background-image: url('".bsa_crop_tool($crop, bsa_upload_url(null, $ad['img']), 400, 170)."')\"></div>"; // -- ITEM -- IMG

		echo '</div>'; // -- END -- ITEM THUMB

		if ( $ad['url'] != '' && $ad['url'] != '#' ) {
			echo '<span class="bsaProItemInner__url" style="color:'.bsa_space($sid, 'ad_url_color').'">'.$url['scheme'].'://'.$url['host'].'</span>'; // -- ITEM -- URL
		}
